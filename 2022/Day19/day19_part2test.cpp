...
int excavateGeodes(int minutes, int ore, int oreRobots, int oreRobCost, int clay, int clayRobots, int clayRobCost, int obsidian, std::array<int, 2>& obsidianRobCost, int obsidianRobots, int geode, int geodeRobots, std::array<int, 2>& geodeRobCost, int oreMax, int clayMax, int obsidianMax, int prevBuilt){
    ...
    if(minutes < 23){                                   // un robot créé la dernière minute n'a aucun intérêt
        ...
        if(clayRobots < clayMax && minutes < 22){   // minutes < 22 car il faut ensuite 1 minute pour un osidian robot et 1 minute pour geode robot
            secondChoice = true;
        }
...
int main(){
    ...
    while(getline(file, s)){
        ...
        int quality{excavateGeodes(1, 0, 1, oreRobotCost, 0, 0, clayRobotCost, 0, obsidianCost, 0, 0, 0, geodeCost, maxOre, maxClay, maxObsidian, 0)};

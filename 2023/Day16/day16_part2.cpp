...
/*
la fonction processMatrix reçoit les matrices en arguments qui sont passées par valeur en C++; 
*/

int processMatrix(std::vector<std::vector<char>> matrix, std::vector<std::vector<std::array<char, 4>>> directions, std::vector<std::vector<char>> energized, int startLign, int startCol, int startDir){
    std::stack<std::array<int, 3>> stackPath;
        stackPath.push({startLign, startCol, startDir});                 // 108 pour l (left), 114 r (right), 116 t (top), 98 b (bottom);
...
int main(){
    ...
    for(int k = 0; k <= matrix[0].size() - 1; k++){                    
        int tempRes{processMatrix(matrix, directions, energized, 0, k, 98)};    //les nodes de la première ligne
        if(tempRes > maxRes){
            maxRes = tempRes;
        }
        int tempSec{processMatrix(matrix, directions, energized, matrix.size() - 1, k, 116)};    //les nodes de la dernière ligne
        if(tempSec > maxRes){
            maxRes = tempSec;
        }
    }
    for(int k = 0; k <= matrix.size() - 1; k++){                     
        int tempRes{processMatrix(matrix, directions, energized, k, 0, 114)};         //les nodes de la première colonne
        if(tempRes > maxRes){
            maxRes = tempRes;
        }
        int tempSec{processMatrix(matrix, directions, energized, k, matrix[0].size() - 1, 108)};         //les nodes de la dernière colonne
        if(tempSec > maxRes){
            maxRes = tempSec;
        }
    }

...
int main(){
    ...
    std::map<int, std::vector<std::string>> blockingElements; // on utilise une hashMap ordonnée par ordre croissant de valeur
    std::set<std::string> disintegrated;
    for(auto& brickCluster : bricksInitial){
        ...
        while(!blocked){
            ...
            disintegrated.insert(std::to_string(brickNumber));      // on ajoute tous les éléments dans un set ordonné par ordre croissant de valeur
        }
        brickNumber++;
    }
    int res{0};
    for(auto& blockNumber : disintegrated){
        std::unordered_set<std::string> fallIfDisintegrated;
        fallIfDisintegrated.insert(blockNumber);
        for(auto& [key, val] : blockingElements){      // les éléments sont ordonnés par ordre croissant de hauteur de départ
            if(fallIfDisintegrated.find(std::to_string(key)) == fallIfDisintegrated.end()){
                bool falls{true};
                for(auto& sandBrick : val){
                    if(fallIfDisintegrated.find(sandBrick) == fallIfDisintegrated.end()){ // si un élément est toujours en dessous
                        falls = false;
                    }
                }
                if(falls){
                    std::cout << key << "\n";
                    fallIfDisintegrated.insert(std::to_string(key));
                }
            }
        }
        std::cout << blockNumber << " result " << fallIfDisintegrated.size() - 1 << "\n";
        res += fallIfDisintegrated.size() - 1;
    }
    std::cout << res << "\n";
    return 0;
}
...
int main(){
    ...
    while(getline(file, s)){
        ...
        for(int i = 5; i <= s.length() - 1; i++){
            ...
            if(attributes){
                if(s[i] == 'g'){
                    if(i + 11 != s.length() && s[i + 11] != ','){        // si goldfish supérieur à 9
                        nextAunt = true;
                        break;
                    }
                    ...
                    int attributeVal = s[i + 7] - '0';
                    currAunt.trees = attributeVal;
                    i += 8;
                }
        ...
        if(!nextAunt){                               // on vérifie que les attributs des deux structs sont identiques
            if(currAunt.children == giftAunt.children && currAunt.cats == giftAunt.cats && 
            currAunt.samoyeds == giftAunt.samoyeds && currAunt.pomeranians == giftAunt.pomeranians && 
            currAunt.akitas == giftAunt.akitas && currAunt.vizslas == giftAunt.vizslas && 
            currAunt.goldfish == giftAunt.goldfish && currAunt.trees == giftAunt.trees && 

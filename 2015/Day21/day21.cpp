        ...
        for(int j = 0; j <= armors.size() - 1; j++){
            ...
            for(int k = 0; k <= rings.size() - 2; k++){
                ...
                for(int m = k + 1; m <= rings.size() - 1; m++){
                    ...
                    if(std::ceil(bossHitPoints / damageToBoss) <= std::ceil(playerHitPoints / damageToPlayer)){ // s'il faut autant ou moins de coups pour tuer le boss que de coups nécessaires au boss pour tuer le joueur
                        if(goldSpent < res){
                            res = goldSpent;
                        }
                    }
